    ...
    static class SchemaEncoder
    {
        ...
        class LookupTable : List<LookupItem>
        {
            ...
            private readonly Dictionary<
                KeyValuePair<Thrift.Type, Thrift.ConvertedType>,
                SType
            > _typeAndConvertedTypeToType = new();
            ...
            private void AddSystemTypeInfo(
                Thrift.Type thriftType,
                SType t,
                Thrift.ConvertedType? ct,
                int priority
            )
            {
            ...
            public void Add(Thrift.Type thriftType, SType t, int priority, params object[] options)
            {
                ...
                for (int i = 0; i < options.Length; i += 2)
                {
                    ...
                    _typeAndConvertedTypeToType.Add(
                        new KeyValuePair<Thrift.Type, ConvertedType>(thriftType, ct),
                        clr
                    );
            ...
            public bool FindTypeTuple(
                SType type,
                out Thrift.Type thriftType,
                out Thrift.ConvertedType? convertedType
            )
            {
        ...
        [Obsolete]
        private static readonly Dictionary<SType, DataType> _systemTypeToObsoleteType =
            new()
            {
                ...
                { typeof(Google.Protobuf.ByteString), DataType.ByteArray }
            };

        private static readonly LookupTable _lt =
            new()
            {
                { Thrift.Type.BOOLEAN, typeof(bool) },
                {
                    Thrift.Type.INT32,
                    typeof(int),
                    Thrift.ConvertedType.UINT_8,
                    typeof(byte),
                    Thrift.ConvertedType.INT_8,
                    typeof(sbyte),
                    Thrift.ConvertedType.UINT_16,
                    typeof(ushort),
                    Thrift.ConvertedType.INT_16,
                    typeof(short),
                    Thrift.ConvertedType.UINT_32,
                    typeof(uint),
                    Thrift.ConvertedType.INT_32,
                    typeof(int),
                    Thrift.ConvertedType.DATE,
                    typeof(DateTime),
                    Thrift.ConvertedType.DECIMAL,
                    typeof(decimal),
                    Thrift.ConvertedType.TIME_MILLIS,
                    typeof(TimeSpan),
                ...
                },
                {
                    Thrift.Type.INT64,
                    typeof(long),
                    Thrift.ConvertedType.INT_64,
                    typeof(long),
                    Thrift.ConvertedType.UINT_64,
                    typeof(ulong),
                    Thrift.ConvertedType.TIME_MICROS,
                    typeof(TimeSpan),
                ...
                },
                {
                    ...
                    Thrift.ConvertedType.DECIMAL,
                    typeof(decimal),
                    Thrift.ConvertedType.INTERVAL,
                    typeof(Interval)
                },
                { Thrift.Type.BYTE_ARRAY, typeof(Google.Protobuf.ByteString) }
            };

        static bool TryBuildList(
            List<Thrift.SchemaElement> schema,
            ref int index,
            out int ownedChildren,
            out ListField? field
        )
        {
            ...
            field = ListField.CreateWithNoItem(
                tseList.Name,
                tseList.Repetition_type != FieldRepetitionType.REQUIRED
            );
        ...
        )
        {
            ...
            bool isMap =
                root.__isset.converted_type
                && (
                    root.Converted_type == Thrift.ConvertedType.MAP
                    || root.Converted_type == Thrift.ConvertedType.MAP_KEY_VALUE
                );
            if (!isMap)
            {
                ownedChildren = 0;
            ...
            if (tseContainer.Num_children != 2)
            {
                throw new IndexOutOfRangeException(
                    $"dictionary container must have exactly 2 children but {tseContainer.Num_children} found"
                );
            }
        ...
        public static bool IsSupported(SType? t) =>
            t == typeof(DateTime)
            || t == typeof(Google.Protobuf.ByteString)
            || ((t?.IsEnum) ?? false)
            || _lt.IsSupported(t);
        ...
        private static void AdjustEncoding(DataField df, Thrift.SchemaElement tse)
        {
            ...
            else if (df.ClrType == typeof(Google.Protobuf.ByteString))
            {
                tse.Type = Thrift.Type.BYTE_ARRAY;
            }
            else if (df.ClrType.IsEnum)
            {
                tse.Type = Thrift.Type.INT32;
            }
        }
        ...
        private static void Encode(
            ListField listField,
            Thrift.SchemaElement parent,
            IList<Thrift.SchemaElement> container
        )
        {
            ...
            var root = new Thrift.SchemaElement(listField.Name)
            {
                ...
                Repetition_type = listField.IsNullable
        ...
        )
        {
            if (field.SchemaType == SchemaType.Data && field is DataField dataField)
            {
                ...
                Thrift.Type thriftType;
                Thrift.ConvertedType? convertedType;
                if (dataField.ClrType.IsEnum)
                {
                    thriftType = Thrift.Type.INT32;
                    convertedType = Thrift.ConvertedType.INT_32;
                }
                else
                {
                    if (!_lt.FindTypeTuple(dataField.ClrType, out thriftType, out convertedType))
                    {
                        throw new NotSupportedException(
                            $"could not find type tuple for {dataField.ClrType}"
                        );
                    }
                }
                ...
                bool isList =
                    container.Count > 1
                    && container[container.Count - 2].Converted_type == Thrift.ConvertedType.LIST;

                tse.Repetition_type =
                    dataField.IsArray && !isList
        ...
        public static bool FindTypeTuple(
            SType type,
            out Thrift.Type thriftType,
            out Thrift.ConvertedType? convertedType
        ...
        public static SType? FindSystemType(DataType dataType) =>
            (
                from pair in _systemTypeToObsoleteType
                where pair.Value == dataType
                select pair.Key
            ).FirstOrDefault();
