...
namespace Parquet.Encodings
{
    ...
    static class ParquetPlainEncoder
    {
        ...
        )
        {
            ...
            {
                byte[][] resultData = new byte[count][];
                Google.Protobuf.ByteString?[] protoData = (Google.Protobuf.ByteString?[])data;
                for (int i = offset; i < offset + count; ++i)
                {
                    resultData[i - offset] = protoData[i]?.ToByteArray() ?? Array.Empty<byte>();
                }
                Encode(resultData.AsSpan(), destination);
            }
            else if (t.GetElementType()?.IsEnum ?? false)
            {
                Span<int> span = ((int[])data).AsSpan(offset, count);
                Encode(span, destination);
                if (stats != null)
        ...
        public static bool TryEncode(object? value, SchemaElement tse, out byte[]? result)
        {
            ...
            else if (t == typeof(Google.Protobuf.ByteString))
            {
                ...
                result = ((Google.Protobuf.ByteString)value).ToByteArray();
                return true;
            }
            ...
            switch (tse.Type)
            {
                ...
                default:
                    throw new InvalidDataException(
                        $"data type '{tse.Type}' does not represent a decimal"
                    );
            }
        }
        ...
        private static bool TryEncode(DateTime value, SchemaElement tse, out byte[] result)
        {
            switch (tse.Type)
            {
                ...
                default:
                    throw new InvalidDataException(
                        $"data type '{tse.Type}' does not represent any date types"
                    );
            }
        }
        ...
        public static int Decode(Span<byte> source, Span<decimal> data, SchemaElement tse)
        {
            switch (tse.Type)
            {
                ...
                case Thrift.Type.FIXED_LEN_BYTE_ARRAY:
                {
                    ...
                    for (
                        int offset = 0;
                        offset + tl <= source.Length && i < data.Length;
                        offset += tl, i++
                    )
                    {
                    ...
                    try
                    {
                        ...
                        if (isMicros)
                            for (int i = 0; i < longsRead; i++)
                            {
                                ...
                                lv /= 1000;
                                data[i] = lv.AsUnixMillisecondsInDateTime()
